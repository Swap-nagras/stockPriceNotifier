...
def get_change_airtel():
    ...
    if (rate is None):
        rate = soup.find("div", class_="pricupdn nsechange red")
    return rate.text
def get_change_quant_liquid_fund():
    url="https://www.moneycontrol.com/mutual-funds/nav/quant-liquid-plan-direct-plan-growth/MES045"
    content = requests.get(url).text
    soup = BeautifulSoup(content, 'html.parser')
    rate = soup.find("span", class_="green_text")
    if (rate is None):
        rate = soup.find("span", class_="red_text")
    return rate.text
...
change_quant_liquid_fund = clean_text(get_change_quant_liquid_fund())
...
if(change_quant_liquid_fund>0.05 or change_quant_liquid_fund<-0.05):
    ...
    password = os.getenv("PASSWORD")
    subject = get_subject()
    contents = get_contents("Quant Liquid Fund", change_quant_liquid_fund)
    send_email(sender, password, receiver, subject, contents)
sys.exit("script reached end")